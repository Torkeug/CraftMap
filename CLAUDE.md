...
| `tools/pak_extract.py`        | Extracts both disc=0x00 and disc=0x02 files from res.pak using cumulative offset calculation for disc=0x02; `--all` extracts every file in the pak (used to build a full local mirror for reverse-engineering). Known bug (finding 14/15): the disc=0x02 cumulative offset drifts badly by the time it reaches the `prefabs/` subtree, and at least one disc=0x00 entry (`Pathway_Puncher.fbx`) was extracted 13 bytes past its real start. Neither has been root-caused or audited pak-wide — when a file looks wrong, search the raw pak for its expected magic bytes near the computed offset before assuming the *format* (not the extraction) is broken. |
