...
- **129 of 130 shapes from pak_out.** All 11 hull sizes complete. Shape N genuinely exists in-game for 8x3x1 like every other size, but its pak entry has an anomalous format (raw index data at byte 0, no parseable HMD header) that no usable mesh could be derived from — left out of the catalogue (13 of 14 shapes for 8x3x1) rather than shipped with unverified data.
